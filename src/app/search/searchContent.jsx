...
const SearchPage = () => {
  ...
  const filltered = blogs.filter((blog) =>
  ...
  if (filltered.length === 0)
    return <h1 className="text-center text-red-400 text-2xl my-20">empty</h1>;

  return (
    <section>
      <div className="container grid grid-cols-3 max-lg:grid-cols-2 max-md:grid-cols-1 gap-4 mt-15">
        {filltered.slice(0, visibleCount).map((blog) => (
          <Link
      ...
      <button
        ...
        className="flex items-center gap-1 mx-auto my-20 cursor-pointer border-2 border-blue-500 rounded-lg p-3"
      >
